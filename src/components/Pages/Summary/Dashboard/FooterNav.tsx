...
import { FaHeart, FaMap, FaRegHeart, FaSnowflake } from 'react-icons/fa';
import {
...
export default function FooterNav({
  ...
  //   ) {
  //     analyticsEvents('CLICK_PRICE_FREEZE');
  //     router.push(`/summary/freeze/${router.query.id}`);
  //   } else if (
  //     !getPlannetTokenFromLocalStorage() ||
  //     getPlannetUserStatusFromLocalStorage() === 'APPROVED'
  //   ) {
  //     setShowModalPage(true);
  //     setCloningSource('freeze');
  //     setShowLoginModal(true);
  //   } else {
  //     analyticsEvents('CLICK_PRICE_FREEZE');
  //     setGetBalance(true);
  //     router.push(`/summary-details/freeze/${urlParameter}`);
  //   }
  // };
  ...
  const handleBookNow = async () => {
    if (
      ...
      analyticsEvents('CLICK_BOOK');
      router.push(`/summary/ocb/${router.query.id}`);
    } else if (
      !getPlannetTokenFromLocalStorage() ||
      getPlannetUserStatusFromLocalStorage() === 'APPROVED'
    ) {
      setShowModalPage(true);
      setCloningSource('ocb');
      setShowLoginModal(true);
    } else {
      analyticsEvents('CLICK_BOOK');
      // setGetBalance(true);
      router.push(`/summary-details/ocb/${urlParameter}`);
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        tripData.trip.type !== 'AFFILIATE_ITINERARIES' ? (
          <button
            className={`hover:text-[#7440F5] inline-flex flex-col items-center w-1/3`}
            onClick={() => isOpen()}
          >
            <span
              className={`w-16 h-16 rounded-full flex justify-center items-center text-3xl bg-white-100 md:hover:bg-white`}
            >
              <FaMap />
            </span>
            Dashboard
          ...
          <button
            className={`inline-flex flex-col items-center w-1/3`}
            onClick={() => handleSaveForLater()}
          >
            <span
              className={`w-16 h-16 rounded-full bg-white text-[#1F133E] flex justify-center items-center text-3xl
        ...
        <button
          className={`w-1/3 inline-flex flex-col items-center
          ${!isPriceFound ? 'pointer-events-none' : ''}`}
          onClick={handleBookNow}
        >
          <span
            className={`w-16 h-16 rounded-full flex justify-center 
            items-center text-3xl bg-white text-[#1F133E]
            ${!isPriceFound ? 'text-[#FFFFFF66]' : ''}`}
          >
            <BiLike />
          </span>
          Book Now
